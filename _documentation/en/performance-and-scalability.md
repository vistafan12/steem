---
permalink: /documentation/performance-and-scalability/
redirect_from:

title: High Performance and Scalability
subtitle: Steem is capable of handling a larger userbase than Reddit.
image: ../images/frontpage/icon-performance.svg
priority: 10 
summary: >
  <a href="http://expandedramblings.com/index.php/reddit-stats/2/">In 2015 Reddit’s 8.7 million users generated an average of 23 comments per second</a>, with an average of 83 comments per year per user. 
  There were 73 million top-level posts, for an average of 2 new posts per second.  There were about 7 billion up votes creating an average voting rate of 220 votes per second. 
  All told, if Reddit were operating on a blockchain it would require an average of 250 transactions per second. Steem is capable of 1000's of transactions per second.
---

## Overview

Steem is built on the same blockchain technology as BitShares which has publicy demonstrated 1000's of transactions per second on
a distributed network. What follows was adapted from BitShares documentation.

To achieve this industry-leading performance, Steem has borrowed [lessons learned from the LMAX
...
By following these simple rules, Steem is able to process 10,000 transactions per second without any significant
effort devoted to optimization.  
...
To implement a high performance blockchain, STEEM must adopt the same techniques used by LMAX. Several key
...
a transaction on competing networks, such as Ripple and Bitcoin, is about 250 bytes.  A similar transaction on Steem
...
multiple hops on a peer-to-peer network are factored in, it becomes clear that transaction size directly impacts the 
block interval, and therefore the confirmation latency.
